...
public class AppointmentRestService extends BaseRestService {
  ...
  public Response getAppointmentByResponse(@PathParam("id") int id) throws ServiceException {
    final List<AppointmentInfoResp> apptInfoList = new ArrayList<>();
    for (Appointment appointment : appointmentService.findByStatus(new AppointmentStatus(id))) {
      apptInfoList.add(AppointmentInfoResp.parse(appointment, patientIntegrationService,
          doctorIntegService));
    }

    return RESTfulUtil.getOk(apptInfoList);
  }
  ...
  public Response getAppointmentById(@PathParam("id") int id) {
    ...
    try {
      ...
      if (appointment != null) {
        response = RESTfulUtil.getOk(AppointmentInfoResp.parse(appointment,
            patientIntegrationService, doctorIntegService));
      } else {
        response = RESTfulUtil.getNotFound();
      }
    } catch (ServiceException e) {
